...
export abstract class ServerSentEventGenerator {
	...
	public mergeFragments(
		...
		const { eventId, retryDuration, ...renderOptions } =
			options || ({} as Partial<MergeFragmentsOptions>);
		const dataLines = this.eachOptionIsADataLine(renderOptions).concat(
			this.eachNewlineIsADataLine("fragments", data),
		);
	...
	public removeFragments(selector: string, options?: FragmentOptions) {
		const { eventId, retryDuration, ...eventOptions } =
			options || ({} as Partial<FragmentOptions>);
		const dataLines = this.eachOptionIsADataLine(eventOptions).concat(
			this.eachNewlineIsADataLine("selector", selector),
		);
	...
	public mergeSignals(
		...
		const { eventId, retryDuration, ...eventOptions } =
			options || ({} as Partial<MergeSignalsOptions>);
		const dataLines = this.eachOptionIsADataLine(eventOptions).concat(
			this.eachNewlineIsADataLine("signals", JSON.stringify(data)),
		);
	...
	public executeScript(
		...
		const attributesDataLines = this.eachOptionIsADataLine(
			attributes ?? {},
